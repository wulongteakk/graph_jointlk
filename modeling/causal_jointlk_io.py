...
def batchify_examples(
    ...
    for graph_idx, ex in enumerate(examples):
        ...
        rel_name = str(
            ex.get("candidate_relation")
            or ex.get("relation_text")
            or ex.get("relation_type")
            or "UNK"
        ).upper()
        ...
        support_label = float(ex.get("support_label", ex.get("label", 0.0)))
        support_mask_value = float(ex.get("support_mask", ex.get("causal_mask", 1.0)))
        labels.append(support_label)
